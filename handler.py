...
def send_to_queue(book: Book, config: Configuration):
    ...
    sns.publish(TopicArn=topic, Subject="book", Message=book.to_json(), MessageAttributes={"state": {"DataType": "String", "StringValue": book.state}})


def simulateSqsMessage(book: Book) -> dict:
    body = {"Message": book.to_json()}
...
if __name__ == "__main__":
    ...
    while selection != "0":
        ...
        if selection == "1":
            ev = {"body": '{"bookSource":"https://flibusta.is/b/369935", "address":"saint.patricius@gmail.com"}'}
            handle_add_book(ev, "")
        elif selection == "2":
            ...
            book = Book(1, BookState.downloaded, "arn:aws:s3:::books-download-dev/book.epub", "saint.patricius@gmail.com")
            ev = simulateSqsMessage(book)
